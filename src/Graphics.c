...
hScrollbar* createVScrollbar(
    ...
    if (rc == NULL) {
        fprintf(stderr, "Error! RenderContext is null!\n");
        return NULL;
    }
...
void drawHScrollbar(RenderContext* rc, hScrollbar* scrollbar) {
    renderRect(rc, &scrollbar->barRect, &scrollbar->barColor, true);
    renderRect(rc, &scrollbar->handleRect, &scrollbar->handleColor, true);
}
